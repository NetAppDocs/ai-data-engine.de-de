...
Sie führen alle Datenerfassungsaufgaben in AIDE Console durch.

.Bevor Sie beginnen
* Sie benötigen Berechtigungen als _Data Engineer_ oder _Data Scientist_ in AIDE Console (`\https://<cluster_management_ip>/console`).
...
* Die AIDE Premium-Servicelizenz ist installiert und die Inferenzfunktionen sind für Data Curator Funktionalität aktiviert.
...
.Ergebnis
Die Sammlungsdefinition und ihre Einbettungen werden aus AIDE entfernt. Anwendungen, die versuchen, den ehemaligen Abrufendpunkt abzufragen, schlagen nach dem Entfernen der Sammlung fehl.
